...
$(document).ready(function () {
  ...
  var columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: function (params) {
        return '<a style="font-weight:bolder;">' + params.data.INSTALLMENT.ID + '. Taksit</a>';
      },
    },
    {
      headerName: 'TAKSIT TRH.',
      ...
      cellRenderer: function (params) {
        return '<a style="font-weight:bolder;">' + params.data.INSTALLMENT.DT + '</a>';
      }
    },
    {
      headerName: 'TUTAR',
      field: 'INSTALLMENT.PRICE',
      width: 155,
      resizable: false,
      ...
      cellRenderer: function (params) {
        return params.data.INSTALLMENT.PRICE;
      }
    },
    {
      headerName: 'HİZMET',
  ...
  if (document.getElementById("GridService")) {
    ...
    if ($.urlParam('between') == 'true') {
      agGrid.simpleHttpRequest({
        url: "/app-assets/data/app-debtors?between=true&start=" + $.urlParam('start') + "&end=" + $.urlParam('end')
      }).then(function (data) {
        gridOptions.api.setRowData(data);
      });
    } else {
      agGrid.simpleHttpRequest({
        url: "/app-assets/data/app-debtors"
      }).then(function (data) {
        gridOptions.api.setRowData(data);
      });
    }
