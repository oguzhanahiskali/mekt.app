...
$myArray = array();
if($staffMission==1){
	$result = $link->query("SELECT * FROM view_expenses WHERE FIRMA_ID=$user_Firma ORDER BY ID DESC");
}else{
	$result = $link->query("SELECT * FROM view_expenses WHERE FIRMA_ID=$user_Firma AND STAFF_ID = $user_ID ORDER BY ID DESC");
}
if ($result) {
