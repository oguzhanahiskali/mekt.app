...
if(isset($_GET['between'])){
   if($_GET['between']==true && isset($_GET['start']) && isset($_GET['end']) ){
      $startDT = $_GET['start'];
      $endDT = $_GET['end'];
      $result = $link->query("SELECT * FROM view_tahsilat_alinanlar WHERE FIRMA_ID = $user_Firma AND DATE(TAKSIT_TARIHI) between '$startDT' AND '$endDT' ORDER BY TAKSIT_TARIHI DESC");
   }
}else{
   $result = $link->query("SELECT * FROM view_tahsilat_alinanlar WHERE FIRMA_ID = $user_Firma ORDER BY TAKSIT_TARIHI DESC");
}
if ($result) {
