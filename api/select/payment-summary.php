...
$CurrencyExchangeJSON = json_decode(file_get_contents($CurExJSONbase), true);
	...
	$paymentServiceQ = "SELECT * FROM view_paymentServices WHERE FIRMA_ID = '{$user_Firma}'";
	...
	$paymentProductQ = "SELECT * FROM tbl_seans_kart_urun WHERE DURUM = 1 AND FIRMA_ID = '{$user_Firma}'";
	...
	if ( $paymentProductR->rowCount() ){
		foreach( $paymentProductR as $row ){
			if($row['CURRENCY']=='TRY'){
				...
				if($row['BUY_STATUS']==1){ $paymentProductReceive += $exchangeTotal; }
