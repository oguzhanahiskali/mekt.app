    ...
    var columnDefs = [
      {
        ...
        cellRenderer: function(params) {
          return '<a href="session-details.php?CID=' + params.data.CID + '#session-product" style="font-weight:bolder;"><i class="feather icon-bookmark" style="color:#1dab95"></i> ' + params.data.PRODUCT + '</a>'
       }
      },
      {
        headerName: 'FİYAT',
    ...
    if (document.getElementById("GridProduct")) {
      ...
      if ($.urlParam('between') == 'true') {
        agGrid.simpleHttpRequest({
          url: "/api/app-customer/view/report-product.php?MID="+$.urlParam('id')+"&between=true&start=" + $.urlParam('start') + "&end=" + $.urlParam('end')
        }).then(function (data) {
          gridOptions.api.setRowData(data);
        });
      } else {
        agGrid.simpleHttpRequest({
          url: "/api/app-customer/view/report-product.php?MID="+$.urlParam('id')
        }).then(function (data) {
          gridOptions.api.setRowData(data);
        });
      }
