...
if(isset($ids)){
  $query = $db->query("SELECT * FROM view_taksit_sms WHERE FIRMA_ID = '$user_Firma' AND ID = $ids")->fetch(PDO::FETCH_ASSOC);
  if($query){
    $FirmaAdi = $query['FIRMA_ADI'];
    $musteri = $query['MUSTERI'];
    $taksitNo = $query['TAKSIT_ID'];
    $hizmetAdi= $query['HIZMET_ADI'];
    $taksitTarihi= $query['TAKSIT_TARIHI'];
    $borcTutar = $query['FIYAT'].' '.$query['CURRENCY'];
    $musteriTelefon = $query['MUSTERI_TELEFON'];
    $firmaTelefon = $query['FIRMA_TELEFON'];
    ...
    if (in_array(substr($musteriTelefon, 0, 3), $numbersArr)){ $verify = 1; }else{ $verify = 0; }
  
    $Msg = "Merhaba ".$musteri.", ";
    $Msg.= $taksitNo.". Taksit olan ".$hizmetAdi." hizmetinizin ".$taksitTarihi." Son ödeme tarihli, ";
    $Msg.= $borcTutar."  tutarında borcunuz gecikmeye girmiştir. ";
    $Msg.= "Ödeme yapmanız için en kısa sürede salonumuz da beklenmektesiniz. Sağlıklı Günler Dileriz. ";
    $Msg.= "Bilgi İçin: +90".$firmaTelefon." ".$FirmaAdi." B021";
    $MsgList[] =	[
      'number'=> $musteriTelefon,
      'message'=> $Msg,
    ];
  }
  ...
  if($verify==1){
    ...
    $json[] =	[ 'status'=> false, 'wrongNumber'=> $ids['number']];
  }
